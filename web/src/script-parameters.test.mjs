...
assert.deepEqual(validateScriptInputs([
  ...
  { i18nKey: 'validation.parameterNameRequired' },
  { i18nKey: 'validation.parameterNameIdentifier' },
  '',
  { i18nKey: 'validation.parameterNameDuplicate', variables: { line: 3 } },
]);
...
assert.deepEqual(validateScriptInputs([{ name: '__proto__', value: 1 }])[0], { i18nKey: 'validation.parameterNameUnsafe' });
assert.deepEqual(validateScriptInputs([{ name: 'constructor', value: 1 }])[0], { i18nKey: 'validation.parameterNameUnsafe' });
