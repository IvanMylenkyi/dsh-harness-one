...
| Feedback and rich document editing | Partial | `web/src/RichDocEditor.jsx` editor chrome is migrated. Remaining first-party copy is in `web/src/docwall-feedback.jsx` (modal, empty, validation, and accessibility states). | `editors-browser.spec.mjs` covers RichDoc; feedback journey remains |
...
The next migration batch should move through the adapter/validation boundary:
`result-adapter.js`, `variables.js`, `global-variables.js`, and
`schedule-center.js`, while closing the remaining feedback-editor chrome in
`docwall-feedback.jsx`.
