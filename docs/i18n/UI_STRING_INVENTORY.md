...
| Compatibility and locale infrastructure | Explicit compatibility only | `web/src/i18n/legacy.js` and exported `translateText()`/`tx()` remain external-only adapters. Legacy dictionaries are no longer attached to `messages/*.js`; `createTranslator().t()` never performs text lookup, and `check:i18n` rejects `tx()`/`translateText()` in first-party source. The production call-site audit found no such calls outside the compatibility module. | Dictionary/manifest parity, legacy-boundary assertions, stable-key scanner, and browser language smoke |
...
The root test entry is Node-based and Windows-portable; the package installer,
storage paths, script workspace, symlink-optional tests, and Lark Desktop CLI
fixture now adapt to Windows. Optional orchestrator suites still depend on
their package install (`ajv`, `cron-parser`, `@deepseek-ai/schemastery`) and
cannot be reported as fully green without those prerequisites. Do not translate node labels, documents,
